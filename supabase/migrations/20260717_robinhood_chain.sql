...
--
-- The game now runs on Robinhood Chain (an Ethereum L2; mainnet chain id
-- 4663). $FIGHT10 is a standard ERC-20 and wallets
...
--      join_pvp_match's login-wallet == deposit-wallet check now lowercases
--      both sides (base58 Solana keys were case-sensitive; 0x keys are not).
--
--   2. matches.pot_tokens switches from RAW UNITS to WHOLE TOKENS. The old
--      6-decimal raw value (2500 × 10^6 per entry) fit in a bigint, but a
--      standard 18-decimal ERC-20 entry (2500 × 10^18) overflows bigint.
--      join_pvp_match / leave_my_matches now add/subtract 2500 per entry, and
--      existing rows are backfilled (divided by 10^6). The payouts ledger is
--      untouched — its amount_raw stays raw and carries its own decimals.
--
-- Safe to run (and re-run) on an existing database. Run in the Supabase SQL
-- editor. Kept in sync with fresh_setup.sql §4/§5.
-- ---------------------------------------------------------------------------

-- Backfill BEFORE swapping the functions: convert historical 6-decimal raw
-- pots to whole tokens. Idempotent guard: a raw pot is always a multiple of
-- 2500 × 10^6, so anything ≥ 10^6 is old-format (whole-token pots max out at
-- 25,000 for a 10-player lobby).
update public.matches
set pot_tokens = pot_tokens / 1000000
where pot_tokens >= 1000000;

-- Payout amounts stay in RAW units (each row carries its own decimals), but
-- an 18-decimal ERC-20 payout (e.g. 4,500 × 10^18) overflows bigint — widen
-- to numeric. Existing 6-decimal rows keep their values and decimals=6; new
-- rows default to the ERC-20 standard 18.
alter table public.payouts alter column amount_raw type numeric(38,0);
alter table public.payouts alter column decimals set default 18;

-- ---------------------------------------------------------------------------
-- join_pvp_match — deposit-before-join, verified server-side (f10join checks
-- the ERC-20 Transfer on-chain BEFORE calling this). Changes from the
-- previous version: lower()ed wallet comparison, pot in whole tokens.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- leave_my_matches — pot refund bookkeeping now in whole tokens too.
-- ---------------------------------------------------------------------------
