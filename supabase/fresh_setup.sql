...
-- ===========================================================================
-- ROBINHOOD CHAIN FINAL STATE (authoritative). The game runs on Robinhood
-- Chain (an Ethereum L2, chain id 4663): $FIGHT10 is a standard ERC-20 and
-- wallets / tx hashes are case-insensitive 0x hex. The EVM-era definitions
-- above are therefore the live ones — the lower()ed wallet + deposit
-- comparisons in join_pvp_match (§20), the lower(deposit_tx) unique indexes
-- (#7), the nonce-based payout guard (matches.payout_nonce) and
-- payouts.decimals defaulting to 18. Mirrors
-- migrations/20260905_robinhood_chain_return.sql, which additionally carries an
-- existing (Solana-era) database across; see that migration's header for the
-- rationale. The Solana build's matches.payout_blockhash column is not created
-- on a fresh install (it is left in place, unused, on migrated databases).
-- ===========================================================================


-- ===========================================================================
-- 20. Payout lifecycle status + durable job queue (mirrors migration
--     20260801). Part A of that migration (deadline-to-snapshot) is applied
--     in place in the function bodies above.
-- ===========================================================================
-- ===========================================================================
