...
-- Blockhash the payout was signed with (Solana's replace for payout_nonce).
alter table public.matches
  add column if not exists payout_blockhash text;

-- Base58 signatures are case-sensitive — drop the case-insensitive deposit
-- indexes; the exact unique constraints (consumed_deposits PK, match_players
-- unique(deposit_tx)) are the guard.
...
-- New payout rows default to 9 decimals (SPL). Existing rows keep their value.
alter table public.payouts alter column decimals set default 9;

-- join_pvp_match — case-sensitive base58 comparisons (no lower()).
create or replace function public.join_pvp_match(
  p_user_id        uuid,
  p_max_players    smallint default 2,
  p_deposit_tx     text     default null,
...
declare
  ...
  v_share        int;
  v_dur          int;
  -- Base58 is CASE-SENSITIVE — no lower() (see the Solana migration).
...
begin
  ...
  -- One wallet per player: deposit signer MUST equal the signed-in wallet.
  -- Compare only the trailing base58 segment, CASE-SENSITIVELY (no lower()).
  if v_login_wallet is null
