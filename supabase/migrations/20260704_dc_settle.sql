-- ============================================================================
-- Disconnect settlement (re-instated): the SURVIVING player wins.
--
-- Previously a disconnect could leave the match as a no-contest (disputed), so
-- the player who outlasted a disconnected rival never got the win. This restores
-- disconnect-aware settlement (heartbeat-based):
--   • disconnected players (stale heartbeat) don't block settlement, and
--   • are excluded from winner selection (a DC is a loss, not a ban), and
--   • when everyone but one has disconnected with no combat, that lone player
--     wins by WALKOVER instead of the match being disputed.
--
