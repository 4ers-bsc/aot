...
The app runs on Robinhood Chain **mainnet only**. The parameters are defined
once in [`src/network.js`](src/network.js) for the client and mirrored in each
Supabase edge function (`f10join`, `f10treasurer`, `f10admin`) — they drive
the wallet's add/switch-chain prompts, the read RPC, and every explorer link.
...
### Client (Vite build-time env vars, all optional overrides)

The token and escrow addresses default to placeholders in
[`src/main.js`](src/main.js) — set them via env vars at build time (a local
`.env` file or your hosting platform's environment settings), or edit the
constants directly.
...
| `VITE_ROBINHOOD_RPC_URL` | Override the public RPC (e.g. a dedicated key) | mainnet RPC |
...
| `RPC_URL`, `RPC_URL_2`, `RPC_URL_3` | all three | Optional RPC pool (round-robin + failover); falls back to the public mainnet RPC |
...
1. **Deposit** — the client puts the wallet on Robinhood Chain
   (`wallet_switchEthereumChain`, adding it from the network table if needed)
