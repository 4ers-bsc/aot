...
//
// Chain: Robinhood Chain (an Ethereum L2, chain id 4663). The app runs on
// mainnet only — the network is defined below and matches the client's
// src/network.js.
//
// The escrow must hold a little ETH on Robinhood Chain to pay gas for payouts.
//
// Two entry points:
...
// client; the ops dashboard's Deployment tab flags a client/server mismatch.
// ---------------------------------------------------------------------------
...
// f10admin — edit all together if the deposit format changes.
...
async function processPayout(opts: {
  ...
  try {
    ...
    // fall through to the slot claim, which auto-releases them if >10 min old;
    // with hash+nonce persisted BEFORE broadcast, a 'pending' row is one that
    ...
    if (snapToken && snapToken !== tokenAddr) {
      return reject("Token contract changed since match entry — payout blocked", 409);
    }
    ...
    if (matchRow.chain_id != null && Number(matchRow.chain_id) !== NETWORK.chainId) {
      return reject("Chain changed since match entry — payout blocked", 409);
    }
    ...
    // write hash+nonce BEFORE broadcasting). Decide what to do by looking at
    ...
    if (recordedTx && recordedTx !== "pending") {
      if (!isTxHash(recordedTx)) {
        ...
        return reject(`Recorded payout ${recordedTx} is not a ${NETWORK.name} transaction — resolve manually`, 409);
      }
    ...
    // confirmed-but-unrelated tx hash cannot pass. All fetches run in parallel
    // to minimise latency.
    ...
    for (let i = 0; i < depositTxs.length; i++) {
      if (!isTxHash(depositTxs[i])) {
        // A deposit recorded on another chain (base58 Solana signature) can't
        // be verified — this match predates the chain move; handle manually.
        return reject(`Deposit ${i + 1} is not a ${NETWORK.name} transaction hash — this match was entered on a previous chain`, 400);
      }
    }
    ...
    for (let i = 0; i < depositTxs.length; i++) {
      ...
      if (!receipt) return reject(`Deposit ${i + 1} not found on-chain`, 400);
      if (receipt.status !== 1) return reject(`Deposit ${i + 1} failed on-chain`, 400);
    ...
    if (reverted) {
      ...
      return reject("Payout tx reverted on-chain: " + payoutSig, 502);
    }
...
async function handleReconcile(req: Request, url: URL, adminClient: any): Promise<Response> {
  ...
  for (const job of (jobs ?? [])) {
    ...
    // because processPayout refuses to re-send while the recorded nonce is
    // spent (the recorded tx may still confirm).
    try {
