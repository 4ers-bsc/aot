-- ============================================================================
-- Deposit signatures are single-use, permanently.
-- ----------------------------------------------------------------------------
-- unique(deposit_tx) on match_players only holds while the seat row exists.
-- leave_my_matches deletes the row when a player leaves a WAITING lobby, which
-- freed the signature — a modified client that saved its sig could replay the
-- same on-chain deposit for a fresh seat later, even though the product rule
-- (and the UI) says leaving forfeits the entry fee.
--
-- Fix: a permanent consumed_deposits ledger. join_pvp_match records every
-- signature the moment it takes a seat and rejects any signature seen before.
-- Rows are never deleted. The honest retry flow is unaffected: a player whose
-- join succeeded but whose response was lost is returned their existing seat
-- by the rejoin block BEFORE the consumed check runs, and a join that failed
-- before seating consumed nothing.
--
-- Safe to run on an existing project (idempotent, with backfill).
-- ============================================================================

create table if not exists public.consumed_deposits (
...
-- Service-role only: RLS on with no policies blocks all direct client access.
alter table public.consumed_deposits enable row level security;

-- Backfill every signature already recorded on a seat so historical deposits
-- can't be replayed either.
insert into public.consumed_deposits (deposit_tx, user_id, match_id)
select deposit_tx, user_id, match_id
from public.match_players
where deposit_tx is not null
on conflict (deposit_tx) do nothing;

-- ----------------------------------------------------------------------------
-- join_pvp_match — same as 20260707_join_releases_dead_match.sql plus the
-- consumed-deposit check and recording. Kept in sync with fresh_setup.sql §4.
-- ----------------------------------------------------------------------------
...
-- Service-role only, as before (idempotent re-grant).
