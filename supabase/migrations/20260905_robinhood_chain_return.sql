-- ============================================================================
-- Migration: Solana → Robinhood Chain (Ethereum L2, chain id 4663).
--
-- The game runs on Robinhood Chain again. $FIGHT10 is a standard ERC-20 and
-- wallets are 0x addresses, so this reverses the case-sensitivity the Solana
-- migration (20260731) introduced and returns payouts to the nonce-based
-- double-pay guard. Four consequences:
--
--   1. Wallet + deposit comparisons are CASE-INSENSITIVE again. EIP-55 only
--      varies the case of an Ethereum address, and EVM tx hashes are
--      case-insensitive hex, so join_pvp_match lower()s both sides of the
--      login-wallet == deposit-wallet check, stores deposit_tx lowercased, and
--      checks the replay ledger case-insensitively (as 20260724 / 20260730 did).
--
--   2. The functional unique indexes on lower(deposit_tx) are RECREATED so a
--      re-cased hash can never pass replay protection as a new deposit.
--
--   3. Payout double-spend protection is nonce based again: matches.payout_nonce
--      records the escrow nonce a payout was signed with (written with the hash
--      BEFORE broadcast, see f10treasurer / f10admin). A retry that finds the
--      hash un-mined only re-sends while that nonce is still FREE on-chain — a
--      re-send then reuses the same nonce, so at most one transfer can ever
--      confirm. matches.payout_blockhash (Solana) is left in place, unused, for
--      backward compatibility.
--
--   4. payouts.decimals default flips 9 → 18 (standard ERC-20). Existing rows
--      keep their stored decimals; only new rows default to 18.
--
-- pot_tokens stays in WHOLE tokens — no raw-unit rescale is needed. Solana-era
-- rows (base58 signatures / wallets) are untouched: they belong to matches
-- that settled on the previous chain, and the payout functions refuse to
-- reconcile a non-0x deposit or payout hash rather than misread it. Safe to
-- run (and re-run) on an existing database. Run in the Supabase SQL editor as
-- the role that owns the functions. Kept in sync with fresh_setup.sql.
-- ============================================================================

begin;

-- 3. Nonce the payout was signed with (present since 20260730; idempotent).
alter table public.matches
  add column if not exists payout_nonce int;

-- 1/2. Any 0x deposit hash stored in mixed case is normalised BEFORE the
-- case-insensitive unique indexes go back on. Base58 (Solana-era) signatures
-- never match the 0x pattern and are left exactly as stored.
update public.consumed_deposits
   set deposit_tx = lower(deposit_tx)
 where deposit_tx ~* '^0x[0-9a-f]{64}$' and deposit_tx <> lower(deposit_tx);
...
-- 4. New payout rows default to 18 decimals (ERC-20). Existing rows keep theirs.
alter table public.payouts alter column decimals set default 18;

-- 1. join_pvp_match — case-insensitive EVM comparisons (lower()). Body is
--    otherwise identical to 20260731_solana_migration.sql / fresh_setup.sql §20.
...
declare
  ...
  v_share        int;
  v_dur          int;
  -- P1: economic-identity snapshot, normalised to a bare lowercased 0x address
...
begin
  ...
  -- EVM tx hashes are case-insensitive — store/compare the lowercased value.
  v_deposit_tx := lower(trim(p_deposit_tx));

  -- Serialise concurrent joins by the same user so two requests can't both pass
  -- the membership check below and take two seats.
  perform pg_advisory_xact_lock(hashtext('fight10_join_user:' || v_uid::text));

  -- Refresh the profile (display name + login wallet) for this user. A requested
  -- name already held by another wallet is ignored — joining must never fail
  -- over a username.
  ...
  -- only when that match is genuinely still live.
  ...
  -- Deposits are single-use, permanently. Case-insensitive on the hash.
  ...
  -- Global capacity cap (serialised so a burst of joins can't overshoot).
  ...
  if found then
    -- Gap-resilient seat assignment: the lowest free seat number.
    ...
    -- Activate once every seat is filled.
  ...
  -- IDENTITY (token contract, chain id, escrow wallet) that apply RIGHT NOW so
  -- the payout path verifies deposits and pays out against the exact contract
  -- the players deposited under, never a later config / redeploy.
